...
public class SpiderDungeonEggRoomPiece extends SpiderDungeonPiece {
    ...
    public boolean generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator generator, Random random, BlockBox box, ChunkPos chunkPos, BlockPos blockPos) {
        ...
        this.addBlock(world, Blocks.WHITE_WOOL.getDefaultState(), chestPos.getX() + 1, chestPos.getY(), chestPos.getZ(), box);
        this.addBlock(world, Blocks.WHITE_WOOL.getDefaultState(), chestPos.getX() - 1, chestPos.getY(), chestPos.getZ(), box);
        this.addBlock(world, Blocks.WHITE_WOOL.getDefaultState(), chestPos.getX(), chestPos.getY(), chestPos.getZ() + 1, box);
        this.addBlock(world, Blocks.WHITE_WOOL.getDefaultState(), chestPos.getX(), chestPos.getY(), chestPos.getZ() - 1, box);
        this.addBlock(world, Blocks.WHITE_WOOL.getDefaultState(), chestPos.getX(), chestPos.getY() - 1, chestPos.getZ(), box);
        this.addBlock(world, Blocks.WHITE_WOOL.getDefaultState(), chestPos.getX(), chestPos.getY() + 1, chestPos.getZ(), box);
        ...
        if (random.nextFloat() < .6f) {
            this.addChest(world, box, random, chestPos.getX(), chestPos.getY(), chestPos.getZ(), new Identifier(BetterDungeons.MOD_ID, "spider_dungeon/chests/egg_room"));
        } else {
            if (box.contains(chestPos)) {
                this.addBlock(world, Blocks.SPAWNER.getDefaultState(), chestPos.getX(), chestPos.getY(), chestPos.getZ(), box);
                BlockEntity spawnerTileEntity = world.getBlockEntity(chestPos);
                if (spawnerTileEntity instanceof MobSpawnerBlockEntity) {
                    ((MobSpawnerBlockEntity) spawnerTileEntity).getLogic().setEntityId(EntityType.SPIDER);
                } else {
